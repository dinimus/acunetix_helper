...
class ACUX(object):
	...
	def reports(self):
		req = self.__get_request(endpoint="/api/v1/reports")
		return self.__json_return(req.text)

	def downloader(self,endpoint=None):
		req = self.__get_request(endpoint=endpoint)
	...
	def add_target(self,target=None,descr="pentest"):
		req = self.__post_request(endpoint="/api/v1/targets",data={"address":target,"description":descr,"criticality":"10"})
		return self.__json_return(req.text)

	def add_and_start(self,target=None,descr="pentest"):
		target_id = self.add_target(target=target,descr=descr)['target_id']
		payload = {
